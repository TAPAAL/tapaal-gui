...
public final class PetriNetUtil {
	public static final String createGuard(String guard, TAPNPlace target,
		...
		String upperString = guard.substring(guard.indexOf(",") + 1, guard
				.length() - 1);
		String lowerString = guard.substring(1, guard.indexOf(","));
		int upper = upperString.equals("inf") ? Integer.MAX_VALUE : Integer
				.parseInt(upperString);
		int lower = Integer.parseInt(lowerString);

		if (invariantBound < lower
				|| (invariantBound == lower && firstDelim.equals("("))
