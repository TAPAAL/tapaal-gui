...
public class ColoredInhibDegree2Converter implements Degree2Converter {
	...
	private void createTokens(TAPN model, ColoredTAPN degree2Net) {
		for (Token token : model.getTokens()) {
			...
			ColoredToken newToken = new ColoredToken(place, BigDecimal.ZERO, ct
					.getColor());
	...
	private void createTransitions(TAPN model, ColoredTAPN degree2Net)
			throws Exception {
	...
	private void createSimulationOfTransitionOfDegree1(
		...
		if (presetArc instanceof ColoredTransportArc) {
			...
			addTransportArc(degree2Net, presetArc.getSource().getName(), trans,
					targetPlace.getName(), cta.getTimeGuard(), cta
							.getColorGuard(), cta.getPreservation(), cta
							.getOutputValue());
		} else {
			ColoredInputArc inputArc = (ColoredInputArc) presetArc;
			ColoredOutputArc outputArc = (ColoredOutputArc) postsetArc;
			addColoredInputArc(degree2Net, presetArc.getSource().getName(),
					trans, new ColoredInterval(inputArc.getTimeGuard()),
					new ColorSet(inputArc.getColorGuard()));
			addColoredOutputArc(degree2Net, trans, postsetArc.getTarget()
					.getName(), outputArc.getOutputValue());
		}
	...
	private void createArcs(TAPNTransition transition, ColoredTAPN degree2Net)
			throws Exception {
	...
	private void addSimulationOfPresetPostsetPairing(ColoredTAPN degree2Net,
		...
		if (i == transition.getPreset().size()) {
			...
			addColoredOutputArc(degree2Net, String.format(T_MAX_FORMAT,
					transitionName, i), postsetArc.getTarget().getName(),
			...
			addColoredInputArc(degree2Net, presetArc.getSource().getName(),
					trans, new ColoredInterval(presetArc.getTimeGuard()),
					createColorGuardForInputArc(presetArc, postsetArc));
			addColoredOutputArc(degree2Net, trans, pholding, postsetArc
					.getOutputValue());

			trans = String.format(T_I_OUT_FORMAT, transitionName, i);
			addColoredInputArc(degree2Net, pholding, trans,
					new ColoredInterval(), new ColorSet());
			addColoredOutputArc(degree2Net, trans, postsetArc.getTarget()
					.getName(), postsetArc.getOutputValue());
		}

	}

	private void addSimulationOfPresetPostsetPairing(ColoredTAPN degree2Net,
			TAPNTransition transition, int i, ColoredTransportArc presetArc)
			throws Exception {
		String transitionName = transition.getName();
		...
		if (i == transition.getPreset().size()) {
			addTransportArc(degree2Net, presetArc.getSource().getName(), String
					...
					createColorGuardForTransportArc(presetArc), presetArc
							.getPreservation(), presetArc.getOutputValue());
		} else {
			String trans = String.format(T_I_IN_FORMAT, transitionName, i);
			String pholding = String.format(HOLDING_PLACE_FORMAT,
					transitionName, i);
			...
			addTransportArc(degree2Net, presetArc.getSource().getName(), trans,
					pholding, createTimeGuardForTransportArc(presetArc),
					createColorGuardForTransportArc(presetArc), presetArc
							.getPreservation(), presetArc.getOutputValue());

			trans = String.format(T_I_OUT_FORMAT, transitionName, i);
			addTransportArc(degree2Net, pholding, trans, presetArc.getTarget()
					.getName(), createTimeGuardForTransportArc(presetArc),
					createColorGuardForTransportArc(presetArc), presetArc
							.getPreservation(), presetArc.getOutputValue());
		}
	...
	private void createRingStructure(TAPNTransition transition,
		...
		for (int i = 1; i <= (transitionsToCreate - 1) / 2; i++) {
			...
			addPlace(degree2Net, pt, ColoredTimeInvariant.getZeroInvariant(),
					new ColorSet());

			addColoredOutputArc(degree2Net, tiin, pt, 0);
			if (lastPTO != null) {
				addColoredInputArc(degree2Net, lastPTO, tiin,
						new ColoredInterval(), new ColorSet());
			}
			lastPTO = pt;
			...
			String holdingPlace = String.format(HOLDING_PLACE_FORMAT,
					transitionName, i);
			addPlace(degree2Net, holdingPlace, new ColoredTimeInvariant(),
					new ColorSet());
		}
		addColoredInputArc(degree2Net, PLOCK, String.format(T_I_IN_FORMAT,
				transitionName, 1), new ColoredInterval(), new ColorSet());
		...
		addColoredInputArc(degree2Net, lastPTO, tmax, new ColoredInterval(),
				new ColorSet());
		lastPTO = tmax;
		...
		for (int i = (transitionsToCreate - 1) / 2; i >= 1; i--) {
			...
			addColoredInputArc(degree2Net, pt, tiout, new ColoredInterval(),
					new ColorSet());
	...
	private void createOptimizedSimulation(TAPNTransition transition,
		...
		for (Arc arc : transition.getPreset()) {
			if (arc instanceof ColoredTransportArc) {
				...
				addTransportArc(degree2Net, tpa.getSource().getName(), tpa
						...
						tpa.getTarget().getName(), new ColoredInterval(tpa
								.getTimeGuard()), new ColorSet(tpa
								.getColorGuard()), tpa.getPreservation(), tpa
								.getOutputValue());
			} else {
				ColoredInputArc tapnArc = (ColoredInputArc) arc;
				addColoredInputArc(degree2Net, tapnArc.getSource().getName(),
						tapnArc.getTarget().getName(), tapnArc.getTimeGuard(),
		...
		for (Arc arc : transition.getPostset()) {
			if (!(arc instanceof TAPNTransportArc)) {
				addColoredOutputArc(degree2Net, arc.getSource().getName(), arc
						.getTarget().getName(), ((ColoredOutputArc) arc)
						.getOutputValue());
			}
		}
	...
	private void addColoredOutputArc(ColoredTAPN degree2Net, String source,
		...
		ColoredOutputArc arc = new ColoredOutputArc(
				(TAPNTransition) getByName(source),
				(ColoredPlace) getByName(target), outputValue);
		degree2Net.addArc(arc);
	}

	private void addColoredInputArc(ColoredTAPN degree2Net, String source,
			String target, ColoredInterval timeGuard, ColorSet colorGuard)
			throws Exception {
		ColoredInputArc arc = new ColoredInputArc(
				(ColoredPlace) getByName(source),
				(TAPNTransition) getByName(target), timeGuard, colorGuard);
		degree2Net.addArc(arc);
	}
	...
	private void addTransportArc(ColoredTAPN degree2Net, String source,
			...
			ColorSet colorGuard, Preservation preservation, int outputValue)
			throws Exception {
		ColoredTransportArc cta = new ColoredTransportArc(
				(ColoredPlace) getByName(source),
				(TAPNTransition) getByName(intermediate),
				(ColoredPlace) getByName(target), timeGuard, colorGuard,
				preservation, outputValue);
	...
	private void createInitialPlaces(TAPN model, ColoredTAPN degree2Net) {
		for (TAPNPlace place : model.getPlaces()) {
			...
			ColoredTimeInvariant timeInvariant = new ColoredTimeInvariant(cp
					.getTimeInvariant());
	...
	private void addPlace(ColoredTimedArcPetriNet degree2Net, String name,
		...
		ColoredPlace place = new ColoredPlace(name, timeInvariant,
				colorInvariant);
		degree2Net.addPlace(place);
		nameToPTO.put(name, place);
	}

	private ColorSet createColorGuardForInputArc(ColoredInputArc inputArc,
	...
	private ColoredInterval createTimeGuardForTransportArc(
			ColoredTransportArc tarc) {
		ColoredPlace target = (ColoredPlace) tarc.getTarget();

		if (tarc.getPreservation().equals(Preservation.AgeAndValue)
				|| tarc.getPreservation().equals(Preservation.Age)) {
			return new CompositeInterval(tarc.getTimeGuard(), target
					.getTimeInvariant());
		} else {
	...
	private ColorSet createColorGuardForTransportArc(ColoredTransportArc tarc) {
		...
		if (tarc.getPreservation().equals(Preservation.AgeAndValue)
				|| tarc.getPreservation().equals(Preservation.Value)) {
