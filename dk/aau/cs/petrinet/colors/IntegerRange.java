...
public class IntegerRange implements Comparable<IntegerRange> {
	...
	public String convertToTAGuardString(String valueVarName) {
		if (goesToInfinity) {
			...
			return String.format("%1$d <= %2$s  && %2$s <= %3$d", from,
					valueVarName, to);
		}
	}
	...
	public IntegerRange intersect(IntegerRange other) {
		if (isSingle()) { // this: x
			return other.contains(from) ? new IntegerRange(from)
					: emptyIntegerRange();
			...
			return maxFrom > minTo ? emptyIntegerRange() : new IntegerRange(
					maxFrom, minTo);
		} else { // this: x-
			if (other.isSingle()) { // other: z
				return contains(other.from) ? new IntegerRange(other.from)
						: emptyIntegerRange();
			} else if (other.isFiniteRange()) { // other: z-w
				int maxFrom = from > other.from ? from : other.from;
				int minTo = other.to;

				return maxFrom > minTo ? emptyIntegerRange()
						: new IntegerRange(maxFrom, minTo);
			} else { // other: z-
				int maxFrom = from > other.from ? from : other.from;
