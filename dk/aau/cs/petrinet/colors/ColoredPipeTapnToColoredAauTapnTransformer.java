...
public class ColoredPipeTapnToColoredAauTapnTransformer extends
	...
	protected void transformPlace(Place place) {
		...
		ColoredTimeInvariant timeInvariant = new ColoredTimeInvariant(ctp
				.getTimeInvariant().toStringWithoutConstants());

		ColoredPlace tapnPlace = new ColoredPlace(ctp.getName(), timeInvariant,
				colorInvariant);

		for (ColoredToken token : ctp.getColoredTokens()) {
			dk.aau.cs.petrinet.colors.ColoredToken newToken = new dk.aau.cs.petrinet.colors.ColoredToken(
					tapnPlace, token.getAge(), token.getColor().getValue());
	...
	protected void transformTransportArc(TransportArcComponent arc)
		...
		if (arc.getSource() instanceof Place) {
			...
			ColoredInterval timeGuard = new ColoredInterval(cta.getTimeGuard()
					.toStringWithoutConstants());
			ColorSet colorGuard = transformColorGuard(cta.getColorGuard());
			Preservation preserves = Preservation.valueOf(cta.getPreservation()
					.toString());
			int outputValue = cta.getOutputValue().getValue();

			dk.aau.cs.petrinet.colors.ColoredTransportArc tarc = new dk.aau.cs.petrinet.colors.ColoredTransportArc(
					(ColoredPlace) PlaceTransitionObjectBookKeeper.get(cta
							.getSource()),
					(TAPNTransition) PlaceTransitionObjectBookKeeper.get(cta
							.getTarget()),
					(ColoredPlace) PlaceTransitionObjectBookKeeper.get(cta
							.getConnectedTo().getTarget()), timeGuard,
					colorGuard, preserves, outputValue);
	...
	private ColorSet transformColorGuard(pipe.dataLayer.colors.ColorSet colorSet) {
		...
		for (IntOrConstantRange range : colorSet.getRanges()) {
			IntegerRange newRange = new IntegerRange(range
					.toStringWithoutConstants());
	...
	@Override
	protected void transformInhibitorArc(TimedInhibitorArcComponent arc)
			throws Exception {
		pipe.dataLayer.colors.ColoredInhibitorArc cia = (pipe.dataLayer.colors.ColoredInhibitorArc) arc;

		ColoredInterval timeGuard = new ColoredInterval(cia.getTimeGuard()
				.toStringWithoutConstants());
		ColorSet colorGuard = transformColorGuard(cia.getColorGuard());

		ColoredInhibitorArc newArc = new ColoredInhibitorArc(
				(ColoredPlace) PlaceTransitionObjectBookKeeper.get(arc
						.getSource()),
				(TAPNTransition) PlaceTransitionObjectBookKeeper.get(arc
						.getTarget()), timeGuard, colorGuard);
	...
	protected void transformNormalArc(TimedOutputArcComponent arc)
		...
		ColoredOutputArc newArc = new ColoredOutputArc(
				(TAPNTransition) PlaceTransitionObjectBookKeeper.get(arc
						.getSource()),
				(ColoredPlace) PlaceTransitionObjectBookKeeper.get(arc
						.getTarget()), coa.getOutputValue().getValue());
