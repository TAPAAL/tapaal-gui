...
extends PetriNetElementControl {
	...
	protected void paintControl(Graphics g) {
		...
		g2.setColor(
				isSelected() ? Pipe.SELECTION_FILL_COLOUR : Pipe.ELEMENT_FILL_COLOUR
		);
		Ellipse2D.Double ellipse = new Ellipse2D.Double(0, 0, getSize().width - DRAW_OFFSET, getSize().height - DRAW_OFFSET);		
		g2.fill(ellipse);

		g2.setPaint(
				isSelected() ? Pipe.SELECTION_LINE_COLOUR : Pipe.ELEMENT_LINE_COLOUR
		);
		g2.draw(ellipse);
	}
