...
public class VerifytaColoredTraceInterpreter extends VerifytaTraceInterpreter {
	...
	protected TAPNFiringAction interpretTransitionFiring(
		...
		TAPNTransition transition = tapn().getTransitionsByName(
				transitionTranslation.originalTransitionName());
		...
		if (isConcreteTrace) {
			if (transitionTranslation.sequenceInfo().equals(SequenceInfo.WHOLE)) {
				List<TransitionFiring> actions = firingSequence.subList(
		...
		return new dk.aau.cs.petrinet.trace.ColoredTransitionFiringAction(
				transition, consumedTokens, producedTokens);
	}

	private List<ColoredToken> parseProducedTokens(
			List<TransitionFiring> actions) {
		...
		for (int i = actions.size() / 2; i < actions.size(); i++) {
			...
			for (Participant participant : action.participants()) {
				...
				if (!namingScheme().isIgnoredAutomata(automata)
						&& !namingScheme().isIgnoredPlace(targetLocation)) {
					TAPNPlace place = tapn().getPlaceByName(targetLocation);
					BigDecimal clockValue = end.targetState()
							.getLocalClockOrVariable(automata,
									namingScheme().tokenClockName()).lower();
					int color = end.targetState().getLocalClockOrVariable(
							automata, namingScheme().colorVariableName())
							.lower().intValue();

					ColoredToken token = new ColoredToken(place, clockValue,
							color);
					tokens.add(token);
				}
			}
	...
	private List<ColoredToken> parseProducedTokens(TransitionFiring start,
		...
		for (Participant participant : end.participants()) {
			...
			if (!namingScheme().isIgnoredAutomata(automata)
				...
				BigDecimal clockValue = end.targetState()
						.getLocalClockOrVariable(automata,
								namingScheme().tokenClockName()).lower();
				int color = end.targetState().getLocalClockOrVariable(automata,
						namingScheme().colorVariableName()).lower().intValue();
	...
	private List<ColoredToken> parseConsumedTokens(
			List<TransitionFiring> actions) {
		ArrayList<ColoredToken> tokens = new ArrayList<ColoredToken>();
			...
			for (Participant participant : action.participants()) {
				...
				if (!namingScheme().isIgnoredAutomata(automata)
					...
					BigDecimal clockValue = participant.clockOrVariableValue(
							namingScheme().tokenClockName()).lower();
					int color = participant.clockOrVariableValue(
							namingScheme().colorVariableName()).lower()
							.intValue();
	...
	private List<ColoredToken> parseConsumedTokens(TransitionFiring start,
			TransitionFiring end) {
		ArrayList<ColoredToken> tokens = new ArrayList<ColoredToken>();
		...
		for (Participant participant : end.participants()) {
			...
			if (!namingScheme().isIgnoredAutomata(automata)
				...
				BigDecimal clockValue = start.sourceState()
						.getLocalClockOrVariable(automata,
								namingScheme().tokenClockName()).lower();
				int color = start.sourceState().getLocalClockOrVariable(
						automata, namingScheme().colorVariableName()).lower()
						.intValue();
