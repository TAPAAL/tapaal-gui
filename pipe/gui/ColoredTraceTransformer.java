...
public class ColoredTraceTransformer extends TraceTransformer {
	...
	protected pipe.dataLayer.FiringAction transformDiscreteTransition(
		...
		if (action instanceof dk.aau.cs.petrinet.trace.ColoredTransitionFiringAction) {
			...
			pipe.dataLayer.TimedTransitionComponent transition = (pipe.dataLayer.TimedTransitionComponent) model()
					.getTransitionByName(transitionFiringAction.transition());
			firingAction = new pipe.dataLayer.ColoredDiscreteFiringAction(
					transition);

			if (isConcreteTrace) {
				convertAndAddConsumedTokens(firingAction,
						transitionFiringAction.consumedTokens());
				convertAndAddProducedTokens(firingAction,
						transitionFiringAction.producedTokens());
			}
		}
	...
	private void convertAndAddProducedTokens(
		...
		for (dk.aau.cs.petrinet.colors.ColoredToken aauToken : producedTokens) {
			pipe.dataLayer.colors.ColoredTimedPlace place = (pipe.dataLayer.colors.ColoredTimedPlace) model()
					.getPlaceByName(aauToken.place().getName());
			BigDecimal age = aauToken.age();
			int color = aauToken.getColor();
			pipe.dataLayer.colors.ColoredToken pipeToken = new pipe.dataLayer.colors.ColoredToken(
					age, color);
	...
	private void convertAndAddConsumedTokens(
			pipe.dataLayer.ColoredDiscreteFiringAction firingAction,
			List<dk.aau.cs.petrinet.colors.ColoredToken> consumedTokens) {
