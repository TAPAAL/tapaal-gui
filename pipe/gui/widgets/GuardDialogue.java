										 ...
										 */
{
	...
	private void setColoredInitialState(TimedInputArcComponent timedArc) {
		...
		String guard = null;
		if (timedArc instanceof ColoredInputArc) {
			guard = ((ColoredInputArc) timedArc)
					.getColorGuardStringWithoutSetNotation();
		} else if (timedArc instanceof ColoredTransportArc) {
			guard = ((ColoredTransportArc) timedArc)
					.getColorGuardStringWithoutSetNotation();
		} else if (timedArc instanceof ColoredInhibitorArc) {
			guard = ((ColoredInhibitorArc) timedArc)
					.getColorGuardStringWithoutSetNotation();
		}
		...
		if (timedArc instanceof ColoredTransportArc) {
			Preserve preserves = ((ColoredTransportArc) timedArc)
					.getPreservation();
			...
			updateExprTextbox.setText(((ColoredTransportArc) timedArc)
	...
	private void initButtonPanel(final PetriNetObject objectToBeEdited) {
		...
		okButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (!CreateGui.getModel().isUsingColors()) {
					dk.aau.cs.model.tapn.TimeInterval guard = composeGuard(arc
							.getGuard());
					...
					try {
						...
						JOptionPane
								.showMessageDialog(
										...
										String
												.format(
														"There was an error parsing the time interval. The problem concerns \"%1s\".\n Possible causes:\n\t- Use of negative values\n\t- Use of undefined constant",
														e.getMessage()),
										"Error",
					...
					if (arc instanceof ColoredTransportArc) {
						action = ((ColoredTransportArc) arc)
								.setTimeGuard(timeGuard);
					} else if (arc instanceof ColoredInhibitorArc) {
						action = ((ColoredInhibitorArc) arc)
								.setTimeGuard(timeGuard);
					} else {
						action = ((ColoredInputArc) arc)
								.setTimeGuard(timeGuard);
					}
						...
						try {
							...
							JOptionPane
									.showMessageDialog(
											...
											String
													.format(
															"There was an error in parsing the color invariant. The problem concerns \"%1s\".",
															e.getMessage()),
											"Error",
											JOptionPane.INFORMATION_MESSAGE);
							undoManager.undo();
					...
					if (arc instanceof ColoredTransportArc) {
						...
						ColoredTransportArc connectedTo = (ColoredTransportArc) tarc
								.getConnectedTo();
						Command preserveEditConnectedTo = connectedTo
								.setPreservation(preserve);
						undoManager.addEdit(preserveEditConnectedTo);

						IntOrConstant outputValue = null;
						try {
							outputValue = new IntOrConstant(updateExprTextbox
									.getText());
						} catch (IllegalArgumentException e) {
							JOptionPane
									.showMessageDialog(
						...
						connectedTo = (ColoredTransportArc) tarc
								.getConnectedTo();
						Command outputEditConnectedTo = connectedTo
