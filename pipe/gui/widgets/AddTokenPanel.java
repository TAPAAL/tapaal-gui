...
public class AddTokenPanel extends JPanel {
		...
		okButton.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(normalRadioButton.isSelected()){
					value = new IntOrConstant((Integer)integerValueSpinner.getValue());
				}else{
					value = new IntOrConstant((String)constantDropDown.getSelectedItem());
				}
				
				undoManager.addNewEdit(place.addColoredToken(new ColoredToken(value)));
	...
	private void initOutputValuePanel() {
		...
		outputValuePanel.setBorder(BorderFactory.createTitledBorder("Token Value"));
