...
public class ColoredInterval {
	...
	public ColoredInterval(String lowerParenthesis, IntervalBound lower,
			IntervalBound upper, String upperParenthesis) {
		lowerIncluded = !lowerParenthesis.equals("("); // to make it default to
														// [
		upperIncluded = upper.goesToInfinity() ? false : upperParenthesis
				.equals("]"); // to make it default to )
		this.lower = lower;
		this.upper = upper;
	}

	public ColoredInterval(String interval) {
		this(interval.substring(0, 1), new IntervalBound(interval.substring(1,
				interval.indexOf(",")).trim()), new IntervalBound(interval
				.substring(interval.indexOf(",") + 1, interval.length() - 1)
				.trim()), interval.substring(interval.length() - 1));
	...
	public String toString() {
		...
		return String.format(format, getOpenParenthesis(), lower, upper,
				getCloseParenthesis());
	}

	public boolean contains(ColoredToken token) {
		// check if age fits one of the end points
		if (lower.equals(token) && !lowerIncluded)
			return false;
		if (upper.equals(token) && !upperIncluded)
			return false;

		return lower.isLessThanOrEqual(token)
				&& upper.isGreaterThanOrEqual(token);
	}
	...
	public String toStringWithoutConstants() {
		...
		return String.format(format, getOpenParenthesis(), lower
				.toStringWithoutConstants(), upper.toStringWithoutConstants(),
				getCloseParenthesis());
	}
