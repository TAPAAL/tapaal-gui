...
public class TimedInputArcComponent extends TimedOutputArcComponent {
	...
	public static boolean validateTimeInterval(String timeInterval) {
		if (Pattern.matches(
			...
			return Integer.parseInt(firstNumber) <= Integer
	...
	public Command setGuard(TimeInterval guard) {
		...
		return new ArcTimeIntervalEdit(this, oldTimeInterval, inputArc
				.interval());
	}
	...
	public TimedInputArcComponent copy() {
		...
		TimedInputArcComponent timedCopy = new TimedInputArcComponent(copy
				.copy(), this.timeInterval);
		return timedCopy;
	}

	@Override
	public TimedInputArcComponent paste(double despX, double despY,
			boolean toAnotherView) {
		TimedOutputArcComponent copy = new TimedOutputArcComponent(this);
		copy.setSource(this.getSource());
		copy.setTarget(this.getTarget());
		TimedInputArcComponent timedCopy = new TimedInputArcComponent(copy
				.paste(despX, despY, toAnotherView), this.timeInterval);
		return timedCopy;
	}
	...
	public boolean satisfiesGuard(BigDecimal token) {
		...
		// if (token.compareTo(BigDecimal.valueOf(Long.parseLong(
		// partedTimeInteval[0].substring(1) ))) < 0){
		// return false;
		// }
		// }else {
		// if ( token.compareTo(BigDecimal.valueOf(Long.parseLong(
		...
		// if ( partedTimeInteval[1].substring(0,
		// lastIndexOfNumber).contains("inf") ){
		// guardMaxValue = Integer.MAX_VALUE;
		// } else {
		// guardMaxValue = Integer.parseInt( partedTimeInteval[1].substring(0,
		// lastIndexOfNumber) );
		// }
		//		
		...
		// ){
		// if (
		// token.compareTo(BigDecimal.valueOf((Long.parseLong(""+guardMaxValue))))
		// > 0 ){
		// return false;
		// }
		// }else {
