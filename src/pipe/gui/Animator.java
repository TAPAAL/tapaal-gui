...
public class Animator {
	...
	private void setUntimedTrace(TapaalTrace trace) {
		...
		// String transitionName = action instanceof ColoredDiscreteFiringAction
		// ? ((ColoredDiscreteFiringAction)action).getTransition().getName() :
		// ((DiscreetFiringAction)action).getTransition().getName();
	...
	public void stepBack() {
		...
		if (!actionHistory.isEmpty()) {
			//
			// if (actionHistory.get(currentAction) instanceof
			// DiscreetFiringAction){
			// TimedTransitionComponent transition =
			// (TimedTransitionComponent)((DiscreetFiringAction)actionHistory.get(currentAction)).getTransition();
			// // XXX - unsafe cast
			// HashMap<TimedPlaceComponent, ArrayList<BigDecimal>>
			// markingToGoBackTo = markingHistory.get(currentAction);
			//
			// if (markingToGoBackTo == null){
			// System.err.println("No marking to go back to, ERROR!");
			// }
			//
			// HashMap<TimedPlaceComponent, ArrayList<BigDecimal>> presetMarking
			// = new HashMap<TimedPlaceComponent, ArrayList<BigDecimal>>();
			// for (Arc a : transition.getPreset() ){
			// TimedPlaceComponent place = (TimedPlaceComponent)a.getSource();
			// presetMarking.put(place,
			// (ArrayList<BigDecimal>)markingToGoBackTo.get(place).clone());
			// }
			//
			// HashMap<TimedPlaceComponent, ArrayList<BigDecimal>>
			// postsetMarking = new HashMap<TimedPlaceComponent,
			// ArrayList<BigDecimal>>();
			// for (Arc a : transition.getPostset() ){
			// TimedPlaceComponent place = (TimedPlaceComponent)a.getTarget();
			// postsetMarking.put(place,
			// (ArrayList<BigDecimal>)markingToGoBackTo.get(place).clone());
			// }
			//
			// CreateGui.currentPNMLData().fireTimedTransitionBackwards(presetMarking,
			// postsetMarking, transition);
			//
			// //If untimed simulation
			// if (CreateGui.getAbstractAnimationPane() != null){
			//
			// AnimationHistory untimedAnimationHistory =
			// CreateGui.getAbstractAnimationPane();
			// int current = untimedAnimationHistory.getCurrentItem();
			// if
			// ((untimedAnimationHistory.getElement(current-1)).trim().equals(transition.getName())){
			...
			// }
			//
			// }else if(actionHistory.get(currentAction) instanceof
			// ColoredDiscreteFiringAction){
			// CreateGui.currentPNMLData().fireColoredTransitionBackwards((ColoredDiscreteFiringAction)actionHistory.get(currentAction));
			// }else if ( actionHistory.get(currentAction) instanceof
			// TimeDelayFiringAction){
			// BigDecimal timeDelay =
			// ((TimeDelayFiringAction)actionHistory.get(currentAction)).getDealy();
			// try {
	...
	public void stepForward() {
		if (currentAction < actionHistory.size() - 1) {
			tab.network().setMarking(markings.get(currentMarkingIndex + 1));
			// if ( actionHistory.get(currentAction+1) instanceof
			// DiscreetFiringAction){
			// TimedTransitionComponent nextTransition =
			// (TimedTransitionComponent)((DiscreetFiringAction)actionHistory.get(currentAction+1)).getTransition();
			// // XXX - unsafe cast
			//
			// // Before we firer the transition we need to setup Select firing
			// mode, and
			// // firer the transition with the same tokens as in the
			// DiscreetFiringAction
			// // IT IS A HACK TO USE FIRING MODEL SELECT TO THIS !!! -- kyrke
			//
			// //Setup firingmode
			//
			// //If this marking is not saved in marking history (e.g. its a
			// uppaal trace)
			//
			...
			// //
			// // HashMap<TimedPlaceComponent, ArrayList<BigDecimal>>
			// currentmakring = CreateGui.currentPNMLData().getCurrentMarking();
			// // markingHistory.set(currentAction+1, currentmakring);
			// // }
			// //
			// // HashMap<Place, ArrayList<BigDecimal>> consumedTokens =
			// ((DiscreetFiringAction)actionHistory.get(currentAction+1)).getConsumedTokensList();
			// //
			...
			// // int current = untimedAnimationHistory.getCurrentItem();
			// // if
			// ((untimedAnimationHistory.getElement(current)).trim().equals(nextTransition.getName())){
			// //Possible null pointer exception
			// // //It is fired
			...
			// tab.network().setMarking(markings.get(currentMarkingIndex+1));
			//
			// }else if(actionHistory.get(currentAction+1) instanceof
			// ColoredDiscreteFiringAction){
			// ColoredDiscreteFiringAction action =
			// (ColoredDiscreteFiringAction)actionHistory.get(currentAction+1);
			// CreateGui.currentPNMLData().fireTransition(action);
			// }else if (actionHistory.get(currentAction+1) instanceof
			// TimeDelayFiringAction){
			// BigDecimal timeDelay =
			// ((TimeDelayFiringAction)actionHistory.get(currentAction+1)).getDealy();
			//
			// //If this marking is not saved in marking history (e.g. its a
			// uppaal trace)
			//
			...
			// Logger.log("Marking is null, we will fix it");
			// markings.set(currentMarkingIndex+1,
			// currentMarking().delay(timeDelay));
			// }
			//
			// try {
			// tab.network().setMarking(markings.get(currentMarkingIndex+1));
			// } catch (RequireException e) {
			// // XXX - kyrke, An error can con come here as this is valid
			// states stored in animator
			// e.printStackTrace();
			// }
			//				
			// }
	...
	public void fireTransition(TimedTransition transition) {
		...
		// int current = untimedAnimationHistory.getCurrentItem();
		// if
		// ((untimedAnimationHistory.getElement(current)).trim().equals(transition.getName())){
		// //Possible null pointer exception
		// //It is fired
		// untimedAnimationHistory.stepForward();
		// }else{
		// int fireTransition = JOptionPane.showConfirmDialog(
		// CreateGui.getApp().getRootPane(),
