    ...
    companion object {
        ...
        fun past(s: String, tab: TabContent) {
            ...
            for (i in 0 until transition.length) {
                ...
                if (node != null) {
                    val r = tab.guiModelManager.addNewTimedTransitions(tab.model, Point(x + Pipe.PLACE_TRANSITION_HEIGHT, y + Pipe.PLACE_TRANSITION_HEIGHT))
                    if (!r.hasErrors) {
                        nameToElementMap[name] = r.result
                    }
                }
