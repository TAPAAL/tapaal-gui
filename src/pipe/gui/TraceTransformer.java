...
public class TraceTransformer {
	...
	public pipe.dataLayer.TAPNTrace interpretTrace(TAPNTrace trace) {
		pipe.dataLayer.TAPNTrace pipeTrace = new pipe.dataLayer.TAPNTrace(trace
				.isConcreteTrace());
		for (dk.aau.cs.petrinet.trace.TAPNFiringAction action : trace) {
			pipe.dataLayer.FiringAction firingAction = transformFiringAction(
					action, trace.isConcreteTrace());
	...
	protected pipe.dataLayer.FiringAction transformFiringAction(
		...
		if (action instanceof dk.aau.cs.petrinet.trace.TimeDelayFiringAction) {
			BigDecimal delay = ((dk.aau.cs.petrinet.trace.TimeDelayFiringAction) action)
					.delay();
	...
	protected pipe.dataLayer.FiringAction transformDiscreteTransition(
		...
		if (action instanceof dk.aau.cs.petrinet.trace.TransitionFiringAction) {
			...
			pipe.dataLayer.Transition transition = model
					.getTransitionByName(transitionFiringAction.transition());
			List<pipe.dataLayer.simulation.Token> tokens = isConcreteTrace ? convertTokens(transitionFiringAction
					.consumedTokens())
					: null;

			firingAction = new pipe.dataLayer.DiscreetFiringAction(transition,
					tokens);
		}
	...
	private List<pipe.dataLayer.simulation.Token> convertTokens(
		...
		ArrayList<pipe.dataLayer.simulation.Token> tokens = new ArrayList<pipe.dataLayer.simulation.Token>(
				consumedTokens.size());

		for (dk.aau.cs.petrinet.Token aauToken : consumedTokens) {
			pipe.dataLayer.TimedPlaceComponent place = (pipe.dataLayer.TimedPlaceComponent) model
					.getPlaceByName(aauToken.place().getName());
			BigDecimal age = aauToken.age();
			pipe.dataLayer.simulation.Token pipeToken = new pipe.dataLayer.simulation.Token(
					place, age);
