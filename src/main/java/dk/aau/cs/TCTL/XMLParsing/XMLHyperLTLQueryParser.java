...
public class XMLHyperLTLQueryParser extends AbstractXMLQueryParser {
    ...
    public static boolean parse(Node prop, QueryWrapper queryWrapper){
        ...
        XMLHyperLTLQueryParser parser = new XMLHyperLTLQueryParser(prop, queryWrapper);
        queryWrapper.setName(parser.parsePropertyName());
        queryWrapper.setTraceList(parser.getTraces());
    ...
    public static TCTLAbstractProperty parse(Node prop) throws XMLQueryParseException{
        XMLHyperLTLQueryParser parser = new XMLHyperLTLQueryParser(prop);
    ...
    private ArrayList<String> getTraces() {
    ...
    private TCTLAbstractProperty parseFormula(Node property)
        ...
        if(nodeName.equals("invariant")){
            ...
            String traceName = property.getAttributes().item(0).getNodeValue();
            childProperty = parseFormula(child);
            if(childProperty instanceof TCTLAbstractPathProperty){
                return new LTLANode(new TCTLPathToStateConverter((TCTLAbstractPathProperty)childProperty), traceName);
            } else{
                return new LTLANode((TCTLAbstractStateProperty)childProperty, traceName);
            }
        }  else if(nodeName.equals("exists-path")){
            String traceName = property.getAttributes().item(0).getNodeValue();
            childProperty = parseFormula(child);
            if(childProperty instanceof TCTLAbstractPathProperty){
                return new LTLENode(new TCTLPathToStateConverter((TCTLAbstractPathProperty)childProperty), traceName);
            } else{
                return new LTLENode((TCTLAbstractStateProperty)childProperty, traceName);
            }
            ...
            children = getAllChildren(property);
            String trace = property.getAttributes().item(0).getNodeValue();
            if (!traceList.contains(trace)) traceList.add(trace);
            TCTLAbstractProperty pathScopeChild = parseFormula(children.get(0));
