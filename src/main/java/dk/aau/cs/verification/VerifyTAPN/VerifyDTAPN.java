...
public class VerifyDTAPN implements ModelChecker{
	...
	public boolean setup() {
		if (isNotSetup()) {
			...
			int choice = JOptionPane.showOptionDialog(TAPAALGUI.getApp(),
				"The verifydtapn engine is not configured. What would you like to do?",
				"Configure verifydtapn", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE,
	...
	public void downloadEngine() {
		try {
			Path directory = chooseDownloadDirectory("verifydtapn");
			if (directory == null) return;
			Path downloaded = VerificationEngineDownloader.ensureEngine(
				VerificationEngineDownloader.Engine.VERIFYDTAPN, directory);
