...
public class VerifyPNOutputParser extends VerifyTAPNOutputParser{
	...
	public Tuple<QueryResult, Stats> parseOutput(String output) {
		...
		try {			
			...
			Tuple<QueryResult, Stats> value = new Tuple<QueryResult, Stats>(new QueryResult(result, null, queryType, false), new Stats(discovered, explored, explored, transitionStats));
			return value; 
		} catch (Exception e) {
			e.printStackTrace();
		}
