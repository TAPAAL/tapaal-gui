...
public class VerifyTAPNDiscreteVerificationWA implements ModelChecker{
	
	private static final String NEED_TO_LOCATE_VERIFYDTAPNWA_MSG = "TAPAAL needs to know the location of the file verifydtapnWA.\n\n"
			+ "VerifydtapnWA is a part of the TAPAAL distribution and it is\n"
			+ "normally located in the directory lib.";
		
		protected static String verifydtapnWApath = "";
		...
		public VerifyTAPNDiscreteVerificationWA(FileFinder fileFinder, Messenger messenger) {
		...
		public String getVersion() { // atm. any version of VerifyTAPN will do
			...
			if (!isNotSetup()) {
				String[] commands;
				commands = new String[] { verifydtapnWApath, "-v" };
		...
		public boolean isCorrectVersion() {
			if (isNotSetup()) {
				messenger.displayErrorMessage(
						"No verifydtapnWA specified: The verification is cancelled",
						"Verification Error");
				return false;
			}
			...
			if(!file.canExecute()){
				messenger.displayErrorMessage("The engine verifydtapnWA is not executable.\n"
										+ "The verifydtapnWA path will be reset. Please try again, "
										+ "to manually set the verifydtapnWA path.", "Verifydtapn Error");
				resetVerifytapn();
				return false;
			}
		...
		public void setVerifydTapnWAPath(String path) {
			verifydtapnWApath = path;
			Preferences.getInstance().setVerifydtapnWALocation(path);
		}

		public boolean setup() {
			if (isNotSetup()) {
				messenger.displayInfoMessage(NEED_TO_LOCATE_VERIFYDTAPNWA_MSG, "Locate verifydtapnWA");

				try {
					File file = fileFinder.ShowFileBrowserDialog("VerifydtapnWA", "");
					if(file != null){
						if( true ) {//file.getName().matches("^d?verifydtapn.*(?:\\.exe)?$")){
							setVerifydTapnWAPath(file.getAbsolutePath());
						}else{
							messenger.displayErrorMessage("The selected executable does not seem to be verifydtapnWA.");
						}
					}
		...
		private boolean isNotSetup() {
			return verifydtapnWApath == null || verifydtapnWApath.equals("") || !(new File(verifydtapnWApath).exists());
		}
		
		public static boolean trySetup() {

			String verifydtapnWA = null;

			//If env is set, it overwrites the value
			verifydtapnWA = System.getenv("verifydtapnWA");
			if (verifydtapnWA != null && !verifydtapnWA.isEmpty()) {
				if (new File(verifydtapnWA).exists()){
					verifydtapnWApath = verifydtapnWA;
					return true;
				}
			}

			//If pref is set
			verifydtapnWA = Preferences.getInstance().getVerifydtapnWALocation();
			if (verifydtapnWA != null && !verifydtapnWA.isEmpty()) {
				verifydtapnWApath = verifydtapnWA;
				return true;
			}
			...
			String[] paths = {"/bin/verifydtapnWA", "/bin/verifydtapnWA64", "/bin/verifydtapnWA.exe", "/bin/verifydtapnWA64.exe"};
			for (String s : paths) {
				File verifydtapnWAfile = new File(installdir + s);

				if (verifydtapnWAfile.exists()){

					verifydtapnWApath = verifydtapnWAfile.getAbsolutePath();
					return true;
		...
		public VerificationResult<TimedArcPetriNetTrace> verify(VerificationOptions options, Tuple<TimedArcPetriNet, NameMapping> model, TAPNQuery query) throws Exception {	
			if(!supportsModel(model.value1()))
				throw new UnsupportedModelException("VerifydtapnWA does not support the given model.");
...
//				throw new UnsupportedQueryException("Discrete inclusion check only supports upward closed queries.");
		...
		private VerificationResult<TimedArcPetriNetTrace> verify(VerificationOptions options, Tuple<TimedArcPetriNet, NameMapping> model, ExportedVerifyTAPNModel exportedModel, TAPNQuery query) {
			...
			runner = new ProcessRunner(verifydtapnWApath, createArgumentString(exportedModel.modelFile(), exportedModel.queryFile(), options));
			runner.run();
